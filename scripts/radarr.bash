...
curl -s "http://unlimited_$1:7878/api/v3/rootFolder" \
  -X 'POST' \
...
curl -s "http://unlimited_$1:7878/api/v3/config/mediamanagement" \
  -X 'PUT' \
  -H 'Content-Type: application/json' \
  -H "X-Api-Key: $arrApiKey" \
...
curl "http://unlimited_$1:7878/api/v3/metadata/1?" \
  -X 'PUT' \
  -H 'Content-Type: application/json' \
  -H "X-Api-Key: $arrApiKey" \
